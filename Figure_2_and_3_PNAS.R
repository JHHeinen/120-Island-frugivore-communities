...
sapply(c("Cairo","vegan","ape","StatMatch","MASS","ggplot2",
...
# (arrows cannot be plotted in this type of figure directly, so overlay the arrows figure below)
figure2 <- filled.contour(diff12$z, asp = 1, col=col10SUB, 
...
# Save figure 2
CairoPDF("Figure 2.pdf", width=7, height=7)
figure2
dev.off()
...
spp.scrs <- cbind(spp.r, trait = rownames(spp.r))
arrow_factor <- 0.4
axisdata <- as.data.frame(pcoa.all[,c(1,2)]) 

## Figure with arrows only, to overlay with figure 2
figure2arrows <- ggplot(axisdata, aes(Axis.1, Axis.2)) +
  coord_fixed() + 
  geom_density_2d_filled(contour_var = "ndensity",alpha = 0) + 
  geom_point(colour="transparent") +
  theme_classic()+
  geom_segment(data = spp.scrs, 
               aes(x = 0, xend = Axis.1*arrow_factor, y = 0, yend = Axis.2*arrow_factor),
               arrow = arrow(length = unit(0.25, "cm")), colour = "grey60", size = 0.75) +
  geom_text_repel(data = spp.scrs, aes(x = Axis.1*arrow_factor, y = Axis.2*arrow_factor, label = trait),
                  size = 3, col="transparent",fontface = "bold") 
# Show plot
figure2arrows

# Save plot
CairoPDF("Figure 2 arrows.pdf", width=6, height=7.53)
figure2arrows
dev.off()

# Figure with arrows in context
figure2arrowscontext <- ggplot(axisdata, aes(Axis.1, Axis.2)) +
  coord_fixed() + 
  geom_density_2d_filled(contour_var = "ndensity",alpha = 0) + 
  geom_point(colour=nativecol.1, size=0.8) +
  geom_point(colour=aliencol.1, size=0.8) +
  geom_point(colour=extinctcol.1, size=0.8) +
  theme_classic()+
  geom_segment(data = spp.scrs, 
               aes(x = 0, xend = Axis.1*arrow_factor, y = 0, yend = Axis.2*arrow_factor),
               arrow = arrow(length = unit(0.25, "cm")), colour = "grey60", size = 0.50) +
  geom_text_repel(data = spp.scrs, aes(x = Axis.1*arrow_factor, y = Axis.2*arrow_factor, label = trait),
                  size = 3, col="transparent",fontface = "bold") 
# Show plot
figure2arrowscontext 

# Save plot
CairoPDF("Figure 2 arrows in context.pdf", width=6, height=7.53)
figure2arrowscontext
dev.off()
